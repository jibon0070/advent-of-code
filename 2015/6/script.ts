...
const bulbs: number[][] = Array(1000).fill(Array(1000).fill(0));

for (const instruction of input.split("\n")) {
  let command: "turn on" | "turn off" | "toggle";
  ...
  const [[startY, startX], [endY, endX]] = instruction
    .replace(`${command} `, "")
    .split(" through ")
    .map((value) => value.split(",").map(Number));

  for (let y = 0; y < bulbs.length; y++) {
    for (let x = 0; x < bulbs[y].length; x++) {
...
console.log(
  bulbs
    .map((row) => row.reduce((total, bulb) => total + bulb, 0))
